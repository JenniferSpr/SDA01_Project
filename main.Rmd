...
resource_files:
- scripts/cleaning.R
- data/wc2026_qualifier_history.csv
- data/actual_stage.csv
- data/third_place_assignments_495.csv
- data/calendar_ko.csv
- data/elo_ratings.csv
- data/wc_elo_ratings.csv
- data/elo_ratings_wc2026.csv
---
...
Football's global balance of power is constantly shifting: new generations emerge, football cultures develop, and confederations rise or fade in strength. Yet this evolution is easy to miss when teams are judged only by tournament results, which capture a single moment rather than a long run trend. Elo ratings offer a way to track that evolution directly. Indeed, they update continuously based on match outcomes, opponent quality, and game significance, giving a dynamic, historically comparable measure of team strength.

This project uses over a century of historical Elo ratings for national football teams to explore how global football power has shifted across countries, confederations, and time, using the 2026 FIFA World Cup as a focal point. Rather than treating the tournament as an isolated event, we situate it within each team's long run trajectory, asking whether traditional powers still dominate, whether confederation strength is converging, and how debutant or returning nations compare to established qualifiers. Building on this historical analysis, we also develop predictive models to examine team performance and simulate the 2026 FIFA World Cup to explore potential tournament outcomes.

These insights are particularly relevant to football analysts and researchers, sports data scientists, and data-driven sports journalists. The combination of historical analysis, statistical modeling and tournament simulation provides a quantitative framework for examining long-term shifts in competitive strength and assessing team performance in the context of the 2026 FIFA World Cup.
...
This project used historical Elo ratings to examine how global football power has evolved and to place the 2026 FIFA World Cup within this broader context.

The descriptive analysis showed that traditional football powers continue to occupy leading positions, particularly within UEFA and CONMEBOL, while several other confederations have shown signs of improvement over time.

The clustering analysis further showed that qualified teams have followed different historical trajectories.

The predictive analysis confirmed the importance of Elo rating, which was the only statistically significant predictor of reaching at least the Round of 16 in the logistic regression. However, the model also produced both false-positive and false-negative predictions, showing that pre-tournament characteristics cannot fully explain progression.

The simulation reinforced this uncertainty. Although stronger teams generally had higher probabilities of progressing, team strength alone did not determine tournament outcomes. Group composition and tournament pathways also influenced advancement probabilities, and comparison with actual results revealed several outcomes that were relatively unlikely according to the simulations.

However, our approach has several limitations. A football match depends on more than the difference between two Elo ratings. Player form, injuries, red cards, weather, travel and fatigue can all influence performance but are not captured by our models. In addition, modeling choices may influence results.

Overall, the different analyses point to the same central conclusion : Elo rating provides a useful measure of football strength and a strong basis for understanding historical trajectories and estimating future performance, but football remains inherently uncertain.
